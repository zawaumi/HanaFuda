...
- `person` は任意です。未指定なら「さくっと話題」として、現在の状況・会話目的・ユーザーの興味を基にAIが3〜5枚を生成します。相手についての事実は推測しません。
...
- AIの応答はJSONとして解析した後、`DeckGenerateResponse` のスキーマで検証します。
- JSONが壊れている、またはカード数・必須キーが契約に一致しない場合は、JSON形式への修正をAIへ1回だけ依頼します。
- 修正後も契約に一致しない場合は、`502` と `detail` を返します。フロントエンドは生成失敗として扱ってください。
